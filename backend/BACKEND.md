...
Вместо классического CQRS через MediatR, в проекте используется **прямой вызов UseCase-компонентов**. Каждый срез (Feature) изолирует свою логику:
...
Для повышения удобства локального тестирования в `Program.cs` встроен кастомный Middleware. Если запрос поступает из интерфейса документации Scalar и в нем отсутствует заголовок `Authorization`, система **автоматически подставляет тестовый JWT-токен** из конфигурации (`Jwt:TestToken`).
...
Для локального запуска скопируйте `appsettings.json` в `appsettings.Development.json` и заполните параметры. Ниже представлен шаблон конфигурации:
